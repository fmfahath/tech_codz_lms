                    ...
                    <div>
                        {courseDetails?.courseContent.map((chapter, index) => (
                            <div key={index} className='bg-gray-500/5 mb-2 px-4 py-2 shadow rounded'>
                                ...
                                <div className={`${toggle[index] ? "max-h-auto" : "h-0"} overflow-hidden`}>
                                    {chapter.chapterContent.map((lecture, i) => (
                                        <div key={i} className=' flex items-center justify-between pl-5 text-gray-700 mt-2 '>
                                            ...
                                            <div className='w-40 flex items-center gap-5 '>
                                                <Link className='text-blue-500'>Play</Link>
                                                <p className='truncate'>{humanizeDuration(lecture.lectureDuration * 60 * 1000, { units: ["h", "m"] })}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
